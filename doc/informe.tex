...
comportamiento del proxy desarrollado en el ITBA difiera del comportamiento del
mismo proxy en otro lugar generó confusión y dificultó la solución de otros problemas, que
...
\label{remoteConfiguration}

La configuración remota de este proxy HTTP se montó sobre el mismo protocolo HTTP 
ya que ofrecía ciertas facilidades. Por lo tanto es un servicio Web al que se 
accede mediante otro puerto, también configurable en el XML. \\
...
Como un usuario se necesita autenticar para poder cambiar la configuración, se decidió
que no se puedan modificar los usuarios remotamente, a fines de no perder la
autenticación. Se utilizó \textit{HTTP Basic Authentication} para autenticar usuarios, 
siguiendo los lineamientos que expone la RFC 2617. \\

El Web Server implementado es iterativo, es decir, sólo atiende de a un HTTP
Request. Se decidió hacerlo de esta manera ya que no se supone que sea un servicio 
con alta concurrencia, sino que simplemente es accedido por el administrador del 
proxy para modificar las configuraciones del mismo cuando sea necesario. \\

Para que el Web Server tenga cierta extensibilidad, se implementó una versión muy
reducida de la API de Servlets de Java, haciendo que el comportamiento sea familiar
para quien haya usado esa API y quiera implementar algún nuevo servicio en este Web Server.
El mapeo de URLs también es realmente muy simple, sólo basta agregar a un mapa 
el \textit{path} requerido como clave y la clase del Servlet como valor.\\

El Web Server soporta subir archivos al servidor, en este caso se implementó esto
para poder subir nuevos archivos de configuración que reemplacen la configuración 
actual. Sólo se modificará la configuración de los filtros, el resto del archivo 
es ignorado. \\

Este Web Server resultó práctico para exponer también el monitoreo del Proxy HTTP, 
por lo tanto se creó un servlet para el monitoreo también protegido mediante 
autenticación, que expone dicha información.
...
en realidad estaba devolviendo un XML. Esto sucedía en el caso de un request AJAX 
de una imagen, que al no estar autenticado en twitter.com fallaba y la forma de 
informar el error era mediante un XML, pero no enviaba el content type correcto 
(informaba siempre \textit{image/jpeg}). \\
...
A continuación se detalla una guía de instalación para sistemas
\textit{Linux y Mac OS X}.

\begin{enumerate}
  \item En el directorio raíz, se encuentra un archivo \verb+JAR+ con el
  proyecto en su última revisión. En caso en que quiera utilizar este
  \verb+JAR+, diríjase directamente al paso 3. Si desea recompilarlo, diríjase
  al siguiente paso.
  \item Ubicado en el directorio raíz, ejecute\\
  \verb+$> compile.sh+\\
  Esto generará un nuevo \verb+JAR+, sobreescribiendo al ya existente. Si se
  ejecutó con éxito, podrá observar la leyenda\\
  \verb+SUCCESS. Please execute run.sh+
  \item Ejecute\\
  \verb+$> run.sh+\\
  Observará la leyenda\\
  \verb+Starting CSSC HTTP Proxy...+\\
  que indica que el proxy está siendo iniciado. Luego de esto, el proxy estará
  funcionando en los puertos indicados luego de dicha leyenda.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Ubicado en el directorio raíz, ejecute\\
  \verb+$> compile.bat+\\
  Esto generará un nuevo \verb+JAR+, sobreescribiendo al ya existente.
  \item Ejecute\\
  \verb+$> run.bat+\\
  El proxy está siendo iniciado. Luego de esto, el proxy estará
  funcionando en los puertos indicados.
\end{enumerate}
...
El proxy es muy flexible en cuanto a configuración. La misma se realiza mediante 
un archivo \verb+config.xml+, ubicado en el directorio en el cual se esté
ejecutando el proxy.\\

Aquí se explica qué permite configurar cada tag válido en el XML:

\begin{itemize}
	\item \verb+<proxyPort>+: Puerto en el cual el Proxy escucha conexiones.
	\item \verb+<remoteServicesPort>+: Puerto en el cual el Proxy corre el 
	servicio Web para configuraciones remotas.
	\item \verb+<threadPoolSize>+: El tamaño del \textit{pool} de \textit{threads}.
	\item \verb+<maxPersistantServerConnections>+: Cantidad máxima de conexiones
	persistentes.
	\item \verb+<maxPersistantServerConnectionsPerServer>+: Cantidad máxima de 
	conexiones persistentes por origin server.
	\item \verb+<loggingFileName>+: Nombre del archivo en donde el Proxy hará
	\textit{logging} de los \textit{requests} y \textit{responses} que vaya 
	procesando.
	\item \verb+<clientKeepAliveTimeout>+: Timeout en las conexiones hacia clientes.
	\item \verb+<serverConnectionPersistentTimeout>+: Timeout en las conexiones 
	persistentes hacia servidores.
	\item \verb+<filters>+: Especifica un conjunto de filtros. Se aplican en el 
	orden explicitados.
	\item \verb+<filter>+: Define un filtro. Un filtro debe tener \verb+<conditions>+
	y \verb+<actions>+.
	\item \verb+<conditions>+: Condiciones para aplicar un filtro. Pueden ser 
	\verb+<OS>+, \verb+<browser>+ y/o \verb+<origin-IPs>+.
	\item \verb+<OS>+: Especifica para qué Sistema Operativo es aplicado el filtro.
	\item \verb+<browser>+: Especifica para qué navegador web es aplicado el filtro.
	\item \verb+<origin-IPs>+: Define un conjunto de IPs para las cuales el filtro
	es aplicado.
	\item \verb+<IP>+: Especifica una dirección IP (En el caso de \verb+<origin-IPs>+ 
	no soporta expresiones regulares).
	\item \verb+<actions>+: Especifica un conjunto de acciones a realizar si se cumplen
	las condiciones anteriormente especificadas.
	\item \verb+<block-all-accesses>+: Se debe especificar un valor \verb+TRUE+ o 
	\verb+FALSE+, que determinará si los usuarios que cumplen las condiciones de este
	filtro pueden acceder a algún contenido o a ninguno (se les bloquea todo acceso).
	\item \verb+<blocked-IPs>+: Especifica un conjunto de IPs a las cuales no se podrá
	acceder si se cumplen las condiciones de este filtro. Es decir, si se coloca la 
	IP de www.itba.edu.ar, no se podrá acceder a ese dominio mientras mantenga esa 
	IP bloqueada.
	\item \verb+<IP>+: Especifica una dirección IP (En el caso de \verb+<blocked-IPs>+
	soporta expresiones regulares: ``24.232.32.*'' es válido como regla).
	\item \verb+<blocked-URIs>+: Especifica un conjunto de URIs a las cuales no se 
	podrá acceder si se cumplen las condiciones de este filtro. Soporta expresiones 
	regulares de estilo ``http://www.google.com/*''.
	\item \verb+<URI>+: Especifica una URI en particular perteneciente al tag 
	\verb+<blocked-URIs>+.
	\item \verb+<blocked-MediaTypes>+: Especifica un conjunto de \textit{Media Types} 
	que son bloqueados a los usuarios que cumplen las condiciones de este filtro.
	\item \verb+<MediaType>+: Especifica un MediaType para el tag \verb+<blocked-MediaTypes>+.
	\item \verb+<max-content-length>+: Especifica el máximo \textit{content-length} 
	que se les permitirá descargar a los usuarios que cumplan las condiciones de 
	este filtro.
	\item \verb+<transform>+: Especifica un conjunto de transformaciones aplicables 
	a los responses de los requests hechos por los clientes.
	\item \verb+<l33t>+: Transformará la response (si la misma es \textit{text/plain}) 
	a formato l33t. 
	\item \verb+<images180>+: Transformará las imágenes aplicándoles una rotación 
	de 180 grados. 
	\item \verb+<admin-users>+: Conjunto de usuarios administradores que tienen 
	autorización para acceder a los servicios de configuración y monitoreo.
	\item \verb+<user>+: Define un usuario dentro de \verb+<admin-users>+.
	\item \verb+<name>+: Define el nombre de usuario de un \verb+<user>+.
	\item \verb+<pass>+: Define la contraseña de un \verb+<user>+.
...
\VerbatimInput{ExamplesXML/config.xml}

Aquí se puede ver un archivo de ejemplo que es válido para ser subido como 
nuevo archivo de configuración (se removió lo ignorado, pero podría estar 
sin problemas):

\VerbatimInput{ExamplesXML/config2.xml}
...
\subsection{Modelado de paquetes \textit{HTTP}}

La primer decisión en cuanto a diseño consistió en definir cómo se
representarían los paquetes \textit{HTTP}.\\

Las clases que modelan estos paquetes pueden encontrarse en el paquete
\verb+http+ y, como se mencionó brevemente en la sección en que se explica el
parseo, consisten en una superclase \verb+HttpPacket+. Ésta encapsula el estado
común de \textit{requests} y \textit{responses}, que son la versión y el
\textit{header} (modelado por la clase \verb+HttpHeader+).\\

\verb+HttpRequest+, que hereda de \verb+HttpPacket+, representa un
\textit{request}. Además del estado obtenido de su clase padre, también contiene
el método del request (en este caso, \verb+GET+, \verb+HEAD+ o \verb+POST+)
representado por la clase \verb+HttpMethod+, y el \textit{path}.\\

\verb+HttpResponse+, también clase hija de \verb+HttpPacket+, es la encargada de
representar un \textit{response}. El estado de esta clase está compuesto por el
\textit{status code} (clase \verb+HttpResponseCode+) y la \textit{reason
phrase}.\\

Como se dijo anteriormente, el \textit{body} de un paquete no se encuentra en
las clases que modelan los paquetes, pues esto implicaba almacenarla en un
arreglo de bytes, con lo que se perdía la funcionalidad provista por el
encabezado \textit{chunked}.

\subsection{Parseo de paquetes \textit{HTTP}}

En el paquete \verb+parsers+ puede verse las clases utilizadas para construir
\verb+parsers+ para los paquetes \verb+HTTP+.\\

La función de estas clases fue descripta en la sección \ref{httpParsing}. En la
raíz del paquete se encuentra la clase \verb+HttpParser+, superclase abstracta,
y sus hijas \verb+HttpRequestParser+ y \verb+HttpResponseParser+. Dentro del
paquete \verb+parsers.lex+ pueden verse las clases generadas por \textit{Lex} a
partir del código fuente presente en el directorio
\verb+src/main/resources/lex+.

\subsection{Núcleo de la aplicación}

En el paquete \verb+net+ se encuentra el núcleo de la aplicación. La misma está
básicamente compuesta por:
\begin{itemize}
  \item Una clase \emph{singleton} \verb+Application+, cuyo método \verb+main+
crea la única instancia de la misma clase y la corre.
  \item Una clase \verb+Logger+, que se encarga de registrar los requests y
  responses que pasan por el proxy. Sólo existe un objeto de este tipo en la
aplicación, y es instanciado por la clase \verb+Application+.
  \item Una clase \verb+ApplicationConfiguration+, que posee principalmente
  métodos para obtener parámetros de configuración. Al igual que el
  \verb+Logger+, sólo existe una instancia de \verb+ApplicationConfiguration+,
  y la misma es instanciada por \verb+Application+.
  \item Una clase \verb+MonitoringService+, también instanciada una única vez
  por \verb+Application+, que contiene las cuentas de la cantidad de bytes
  transferidos y de los bloqueos realizados, y permite acceder a los mismos en
  forma sincronizada.
  \item Una clase \verb+ClientListener+, abstracta, y una clase
  \verb+HttpProxyClientListener+, concreta, que contienen la lógica de un
  servidor \emph{TCP}.
  \item Una serie de interfaces:
  \begin{itemize}
    \item La interfaz \verb+ConnectionManager+, que representa un manejador de
    conexiones entrantes de clientes.
    \item La interfaz \verb+ClientHandler+, que describe clases que puedan
    manejar un \verb+Socket+ resultante de la aceptación de un cliente. La
    diferencia con \verb+ConnectionManager+ es que un \verb+ClientHandler+ es
    aquel que especifica las acciones a efectuar relacionadas con la lógica de
    la aplicación, mientras que un \verb+ConnectionManager+ se limita a
    administrar los \verb+Socket+s para proveer concurrencia.
    \item La interfaz \verb+ServerManager+, que especifica métodos relacionados
    con la adquisición de \verb+Socket+s para un determinado servidor.
  \end{itemize}
  \item Una serie de implementaciones de las interfaces arriba mencionadas:
  \verb+ThreadPoolConnectionManager+,
  para la interfaz \verb+ConnectionManager+;  \verb+HttpProxyHandler+, para la
  interfaz \verb+ClientHandler+; y finalmente
  \verb+PersistentSemaphorizedServerManager+, para la interfaz
  \verb+ServerManager+.
  \item Un paquete \verb+filters+ que contiene diversas clases relacionadas con
  la aplicación de filtros y transformaciones tanto sobre los requests como
  sobre los responses.
\end{itemize}
 

\subsection{Configuración del proxy}

En el paquete \verb+configuration+ se encuentra la clase
\verb+ConfigurationParser+, encargada de parsear el archivo \textit{XML} de
configuración y poder servirle los parámetros a la clase
\verb+ApplicationConfiguration+.

\subsection{Servidor \textit{HTTP}}

El servidor \textit{HTTP}, utilizado para configuración y monitoreo remotos,
está modelado por las clase del paquete \verb+httpserver+. Dentro de éste, se
encuentra la clase \verb+ApplicationConfigurationServer+, que tiene la lógica
del servidor (aceptación de clientes y atención de sus pedidos).\\

En el paquete \verb+httpserver.servlets+ están las clases que modelan los
\textit{servlets} descriptos en la sección \ref{remoteConfiguration}, y en el
paquete \verb+httpserserver.model+ hay clases que corresponden a autenticación
\textit{HTTP} y a paquetes adaptados al funcionamiento de los \textit{servlets}
(\verb+HttpServletRequest+ y \verb+HttpServletResponse+).

\subsection{Diagramas de clases UML}

Los diagramas \textbf{UML} de las clases que componen al proxy se encuentran 
anexados al final de este informe.
...
\begin{itemize}
	\item Enunciado provisto por la cátedra
	\item RFC 2616, HTTP, \textit{http://tools.ietf.org/html/rfc2616}
	\item RFC 2617, HTTP Authentication, \textit{http://tools.ietf.org/html/rfc2617}
	\item Apache JMeter: \textit{http://jakarta.apache.org/jmeter/}
	\item Apache Bench: \textit{http://httpd.apache.org/docs/2.0/programs/ab.html}
\end{itemize}
